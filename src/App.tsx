...
export default function App() {
  ...
  const totalPL = monthData.filter(([, v]) => v.paidLeave).length;
  ...
  const save = () => {
    const ot = Number(otH || 0) * 60 + Number(otM || 0);
    const hw = Number(hwH || 0) * 60 + Number(hwM || 0);
    const next = { ...records };

    if (ot || hw || paidLeave) next[key] = { overtime: ot, holidayWork: hw, paidLeave };
  ...
  return (
    <div style={S.page}>
        ...
        {Array.from({ length: days }).map((_, i) => {
          ...
          return (
            <div key={k} style={S.day} onClick={() => openInput(k, r)}>
              ...
              {r?.overtime ? <div style={S.ot}>{Math.floor(r.overtime / 60)}h</div> : null}
      ...
      <div style={S.sum}>
        <div>残業 {Math.floor(totalOT / 60)}h</div>
        <div>休出 {totalHW}日</div>
        <div>年休 {totalPL}日</div>
      </div>

      {open && (
        <div style={S.modalBg}>
          <div style={S.modal}>
            <div>残業 <input inputMode="numeric" value={otH} onChange={e => setOtH(e.target.value)} />h</div>
            <div>休出 <input inputMode="numeric" value={hwH} onChange={e => setHwH(e.target.value)} />h</div>
...
const S: any = {
  ...
  week: { display: "grid", gridTemplateColumns: "repeat(7,1fr)", textAlign: "center", fontSize: 11 },
