    ...
    <div style={styles.page}>
      <header style={styles.header}>
        ...
        <div style={styles.summary}>
          ...
          休出 {totalHolidayDays}日 ／ 年休 {totalPaidLeaveDays}日
        </div>
      </header>
        ...
        {Array.from({ length: days }).map((_, i) => {
          ...
          return (
            <div key={key} style={styles.day} onClick={() => openModal(key, rec)}>
              ...
              {rec?.overtime ? <div style={styles.overtime}>{Math.floor(rec.overtime / 60)}h{rec.overtime % 60}m</div> : null}
              {rec?.holidayWork ? <div style={styles.holiday}>休出</div> : null}
              {rec?.paidLeave ? <div style={styles.leave}>年休</div> : null}
            </div>
          );
        })}
      </div>
        ...
        <div style={styles.modalBg}>
          <form style={styles.modal} onSubmit={e => { e.preventDefault(); saveCurrent(); }}>
            <div style={styles.row}>
              残業
              <input type="number" value={otH} min={0} onChange={e => setOtH(+e.target.value)} />h
              <input type="number" value={otM} min={0} step={10} onChange={e => setOtM(+e.target.value)} />m
            </div>
            <div style={styles.row}>
              休出
              <input type="number" value={hwH} min={0} onChange={e => setHwH(+e.target.value)} />h
              <input type="number" value={hwM} min={0} step={10} onChange={e => setHwM(+e.target.value)} />m
            </div>
            <div style={styles.row}>
              <label>
...
const styles: any = {
  ...
  save: { width: "100%", padding: 10, borderRadius: 12, background: "#FFB703", border: "none" },
};
